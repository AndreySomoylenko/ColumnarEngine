...
ByteVector::ByteVector(size_t size, size_t size_in_bytes, void *buf) noexcept
    : capasity_(size_in_bytes), size_(size), size_in_bytes_(size_in_bytes),
      data_(buf) {}
...
void ByteVector::Reserve(size_t size, size_t element_size) {
    Reserve(size * element_size);
}
...
void ByteVector::Push(const void *data, size_t size_in_bytes) {
    ...
    memcpy(static_cast<char *>(data_) + size_in_bytes_,
...
ByteVector::ByteVector(ByteVector &&other)
    : capasity_(other.capasity_), size_(other.size_),
      size_in_bytes_(other.size_in_bytes_), data_(other.data_) {
    other.data_ = nullptr;
    other.size_ = 0;
...
ByteVector::~ByteVector() { free(data_); }
